...
def preprocess_data(df):
    ...
    2. Creates engineered features (contact_intensity, financial_stress).
    ...
    # --- Feature Engineering ---
    # Using validated features from ablation study (new_features_ablation_results.txt)
    # Removed: age_campaign (+0.19% AUC, negligible), is_first_contact (+0.10% AUC, negligible)
    # Added: contact_intensity (+0.56% AUC), financial_stress (+0.43% AUC)
    # Combined improvement: +0.93% AUC
    
    print("Generating engineered features...")
    
    # Contact Intensity: campaign / (pdays + 1)
    # Measures how aggressively this lead is being pursued relative to recency
    # For pdays=999 (never contacted before), this becomes campaign/1000 (very low intensity)
    if 'campaign' in df.columns and 'pdays' in df.columns:
        df['contact_intensity'] = df['campaign'] / (df['pdays'] + 1)
    
    # Financial Stress: count of financial obligations (0-3 scale)
    # Combines housing loan, personal loan, and default status
